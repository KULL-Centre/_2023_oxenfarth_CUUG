...
in_dir     = 'nmr_bundles/' #directory to the bundle pdb structures
out_dir    = 'nmrbundle_data/'
...
di_df = pd.DataFrame()
d_df = pd.DataFrame()
traj_f   = f'{in_dir}/Bundle_{i}_gmx.pdb'
top_f    = f'{in_dir}/Bundle_{i}_gmx.pdb'
traj = md.load_pdb(traj_f)


for k,f in enumerate(traj):
    pdb_tmp  = f'{in_dir}/frame_{k}.pdb'
    out_dir_tmp = f'{out_dir}/rdc_tmp_pf1_{k}.tbl'
...
di_df.to_pickle(f'{out_dir}/nmrbundle_{i}_di_df_pf1.pkl')
d_df.to_pickle(f'{out_dir}/nmrbundle_{i}_d_df_pf1.pkl')
